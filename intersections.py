"""Phase 5 -- boundary crossings (ee/ea/ae/aa) between rounded polygons.
...
E_k (from a^+_k to a^-_{next[k]}). Two polygons cross at four feature combinations:
...
The outersection ordering / Phase-6 overlap integration build on top of these crossings.
"""
...
def segSegParams(p0, u, q0, v):
    ...
    denom = u[0] * v[1] - u[1] * v[0]                 # eps_ab u^a v^b
    if abs(denom) < 1e-14:
        return None
    w = q0 - p0
...
def crossingsAtPair(features, i, j, shift):
    """Boundary crossings between vertex i's two features (arc C_i, edge E_i) and vertex j's
    ...
    out = []
    res = segSegParams(ep0i, eui, ep0j, euj)                       # ee
    if res is not None and 0.0 <= res[0] <= 1.0 and 0.0 <= res[1] <= 1.0:
        out.append((ep0i + res[0] * eui, "ee", i, j))

    for s in lineCircleParams(ep0i, eui, zj, rj):                  # ea: edge i x arc j
        if 0.0 <= s <= 1.0:
            R = ep0i + s * eui
            if onArc(R, zj, rj, mj, cj):
                out.append((R, "ea", i, j))

    for s in lineCircleParams(ep0j, euj, zi, ri):                  # ae: arc i x edge j
        if 0.0 <= s <= 1.0:
            R = ep0j + s * euj
            if onArc(R, zi, ri, mi, ci):
                out.append((R, "ae", i, j))

    for R in circleCirclePoints(zi, ri, zj, rj):                   # aa: arc i x arc j
...
@dataclass
class Crossing:
    """One boundary crossing of polygon A with polygon B. ``point`` is in A's image frame;
    ...
    crossings of a pair by sigmaA gives the order they are met walking A's boundary CCW."""
    point: np.ndarray
    kind: str
    i: int
    j: int
...
def findCrossings(packing, features, neighbors):
    """All boundary crossings between DIFFERENT polygons, driven by the neighbor list.
    ...
    minimum image relative to i. Returns a list of Crossing records, each tagged with its
    ...
    for i, j in neighbors.pairs[~neighbors.sameShape]:
        ...
        for point, kind, _, _ in crossingsAtPair(features, i, j, shift):
            ...
            out.append(Crossing(point, kind, i, j, sigmaA, sigmaB))
    return out

def outersections(packing, crossings):
    """Group crossings by polygon pair and order each group CCW along dA for outersection
    pairing. The pair key is (polyA, polyB) = (shapeId[i], shapeId[j]); within a group the
    crossings are sorted by sigmaA (the CCW boundary coordinate on A), so the outersection of
    the m-th crossing -- the next one met walking dA -- is the (m+1 mod M)-th. Returns a dict
    {(polyA, polyB): [crossings sorted by sigmaA]}.
    """
