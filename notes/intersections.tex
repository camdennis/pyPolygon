...
Two rounded polygons $A,B$ cross at four feature combinations: edge--edge (\textbf{ee}),
...
The crossing formulas place candidate points on the \emph{full} circle $(\vv z_k,\rho_k)$; the
...
A crossing on feature $f$ of $\partial A$ gets the monotonic boundary coordinate
\[
  \boxed{\;\sigma_A = f +
...
\]
Sorting a pair's crossings by $\sigma_A$ gives the order met walking $\partial A$ CCW; the
\emph{outersection} of a crossing is the next one in that cyclic order. It remains to find the
edge parameter $s$ and the arc angle $\phi$ for each crossing.

\section{Edge--edge (ee)}

Edges $\vv P_k(s)=\vv a^{+}_k+s\,\vv u_k$ and $\vv P_\ell(t)=\vv a^{+}_\ell+t\,\vv u_\ell$. With
$\vv w_{k\ell}=\vv a^{+}_\ell-\vv a^{+}_k$, a crossing needs
...
\[
  ...
  t=\frac{\varepsilon_{\alpha\beta}\,w_{k\ell}^{\alpha}u_k^{\beta}}
         {\varepsilon_{\alpha\beta}\,u_k^{\alpha}u_\ell^{\beta}}.
\]
A crossing exists iff the denominator $\varepsilon_{\alpha\beta}u_k^{\alpha}u_\ell^{\beta}\neq0$
...
$\ell$ (an edge can cross one arc twice). Its arc angle is $\phi=\atantwo(\vv R-\vv z_\ell)$
